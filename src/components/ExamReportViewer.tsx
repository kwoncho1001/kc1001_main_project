    ...
    return (
      <div className="space-y-8">
        <button 
          ...
          className="px-6 py-2 bg-background/50 border border-border rounded-xl text-micro hover:bg-accent/10 hover:border-accent/30 transition-all flex items-center gap-2 text-muted-foreground hover:text-accent"
        >
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between mb-12">
        <div>
          <h2 className="text-4xl font-bold heading-tight uppercase mb-2">리포트 보관함</h2>
          <p className="text-muted-foreground font-medium">지금까지 산출된 모든 분석 보고서를 확인할 수 있습니다.</p>
        </div>
        <div className="flex items-center gap-4">
          ...
          <button className="w-12 h-12 rounded-2xl card border-border flex items-center justify-center text-muted-foreground hover:text-foreground transition-all">
            <Filter size={18} />
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {mockReports.map((report) => (
          <button 
            ...
            className="card p-8 relative overflow-hidden group text-left transition-all hover:scale-[1.02] hover:shadow-xl"
          >
            ...
            <div className="relative z-10">
              <div className="flex justify-between items-start mb-8">
                ...
                <span className="text-[10px] font-bold uppercase tracking-widest text-accent px-3 py-1 bg-accent/10 rounded-full">
                  {report.score.toFixed(1)}점
                </span>
              </div>
              <h3 className="text-xl font-bold tracking-tighter uppercase mb-2 text-foreground">{report.title}</h3>
              <div className="flex items-center gap-4 text-[10px] font-bold uppercase tracking-widest text-muted-foreground">
                <span>{report.date}</span>
                <div className="w-1 h-1 rounded-full bg-border"></div>
                <span>{report.student}</span>
              </div>
              <div className="mt-8 flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-accent opacity-0 group-hover:opacity-100 transition-all">
